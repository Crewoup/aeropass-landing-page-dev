...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function prefillProfileData() {
        ...
        if (!selectedModalTitle) {
            ...
            if (firstTitle) {
                firstTitle.classList.remove('bg-captain-card', 'text-gray-400');
                firstTitle.classList.add('active', 'bg-captain-purple', 'text-white', 'font-bold');
    ...
    titlePills.forEach(pill => {
        pill.addEventListener('click', () => {
            titlePills.forEach(p => {
                p.classList.add('bg-captain-card', 'text-gray-400');
                p.classList.remove('active', 'bg-captain-purple', 'text-white', 'font-bold');
            });
            pill.classList.remove('bg-captain-card', 'text-gray-400');
            pill.classList.add('active', 'bg-captain-purple', 'text-white', 'font-bold');
                ...
                airlineDropdown.innerHTML = filtered.map(a => `
                    <div class="dropdown-item p-3 text-sm cursor-pointer hover:bg-white/5 transition-colors border-b border-white/5 last:border-0" data-code="${a.code}">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-[10px] font-mono bg-captain-surface px-1.5 py-0.5 rounded text-gray-400">${a.code}</span>
                        </div>
    ...
    fleetBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            fleetBtns.forEach(b => {
                b.classList.remove('active', 'border-captain-purple/60', 'bg-captain-purple/10', 'ring-1', 'ring-captain-purple/30');
                b.classList.add('border-transparent', 'bg-captain-card/50');
            ...
            btn.classList.add('active', 'border-captain-purple/60', 'bg-captain-purple/10', 'ring-1', 'ring-captain-purple/30');
            btn.classList.remove('border-transparent', 'bg-captain-card/50');
    ...
    function renderPills(fleet) {
        ...
        models.forEach((model, index) => {
            const pill = document.createElement('button');
            pill.className = `aircraft-pill px-4 py-2 rounded-full font-mono text-sm whitespace-nowrap transition-all ${index === 0 ? 'active bg-captain-purple text-white font-bold' : 'bg-captain-card border border-white/10 text-gray-400 hover:bg-captain-card hover:text-white'}`;
            pill.textContent = model;
            
            pill.addEventListener('click', () => {
                // Toggle active state
                document.querySelectorAll('.aircraft-pill').forEach(p => {
                    p.className = 'aircraft-pill px-4 py-2 rounded-full bg-captain-card border border-white/10 text-gray-400 font-mono text-sm whitespace-nowrap hover:bg-captain-card hover:text-white transition-all';
                });
                pill.className = 'aircraft-pill active px-4 py-2 rounded-full bg-captain-purple text-white font-bold font-mono text-sm whitespace-nowrap transition-all';
            });
    ...
    challengeCards.forEach(card => {
        card.addEventListener('click', () => {
            challengeCards.forEach(c => {
                c.classList.remove('active', 'bg-captain-card/80', 'border-captain-purple/60', 'ring-1', 'ring-captain-purple/30');
                c.classList.add('bg-captain-card/30', 'border-white/10');
            });
            card.classList.remove('bg-captain-card/30', 'border-white/10');
            card.classList.add('active', 'bg-captain-card/80', 'border-captain-purple/60', 'ring-1', 'ring-captain-purple/30');
        });
    });
    ...
    function createVisualizerBars() {
        ...
        for (let i = 0; i < barCount; i++) {
            ...
            bar.className = 'w-1 bg-captain-purple/50 rounded-full transition-all duration-100 ease-in-out';
        ...
        if (input && input.value.trim() !== "") {
            ...
            userResponsePlaceholder.innerHTML = `
                <div class="flex items-start gap-3 justify-end">
                    <div class="bg-captain-blue text-white rounded-2xl rounded-tr-none p-4 max-w-[85%]">
