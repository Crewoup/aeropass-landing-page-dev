...
const notifications = [
    ...
    { icon: "fa-plane", text: "A B777 Captain just started the Question Bank mode.", time: "Just now", color: "text-captain-blue" },
    { icon: "fa-users", text: "An A320 FO is currently practicing HR scenarios.", time: "2 mins ago", color: "text-green-400" },
    { icon: "fa-globe", text: "A pilot is currently preparing for an Emirates interview.", time: "5 mins ago", color: "text-captain-purple" },
    ...
    { icon: "fa-clock", text: "12 interview sessions completed in the last hour.", time: "1 hour ago", color: "text-captain-blue" }
];
...
function createToast(data) {
    ...
    toast.className = 'glass-card p-4 rounded-xl flex items-center gap-4 min-w-[320px] max-w-sm border-l-4 border-captain-purple shadow-2xl backdrop-blur-xl bg-captain-bg/80 toast-enter';
    
    toast.innerHTML = `
        <div class="w-10 h-10 rounded-full bg-captain-card flex items-center justify-center ${data.color} font-bold border border-white/10 shadow-inner">
            <i class="fa-solid ${data.icon}"></i>
        </div>
        <div>
            <p class="text-xs font-bold text-white leading-tight mb-1">${data.text}</p>
            <p class="text-[10px] text-gray-400 font-mono">${data.time}</p>
        </div>
    `;
